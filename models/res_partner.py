...
class respartner(models.Model):
    ...
    vat = fields.Char(
        string='TIN', help="Tax Identification Number. Fill it if the company is subjected to taxes. Used by the some of the legal statements.", compute="_compute_vat")
    ...
    def _compute_escola(self):
        for record in self:
            rel_user = record.env['res.users'].search(
    ...
    def _compute_centro_educativo(self):
        for record in self:
            ...
            if rel_user and rel_user.escola != False:
                ...
                with switch(record.escola_id) as e:
                    if e.case('2', True):  # cmontserrat
                        record.centro_educativo = record.env['res.users'].browse(
                            16923).id
                    if e.case('3', True):  # eminguella
                        record.centro_educativo = record.env['res.users'].browse(
                            12794).id
